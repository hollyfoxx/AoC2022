...
class Day5(ElfMachine):
    ...
    def solve_second_puzzle(self, input_data: str) -> str:
        ...
        for instruction in instruction_section.split("\n"):
            ...
            # perform moves
            moving_crates = crate_map[int(from_stack) - 1][-int(move_count) :]
            crate_map[int(from_stack) - 1] = crate_map[int(from_stack) - 1][
                : -int(move_count)
            ]
            crate_map[int(to_stack) - 1].extend(moving_crates)
